...
def vendor(tmp_path: Path) -> Path:
    directory = tmp_path / "src" / "ksef_mcp" / "vendor"
...
def test_a_transfer_cut_in_half_is_named_as_a_short_file(vendor: Path) -> None:
    (vendor / "src/ksef_mcp/vendor/ksef-fe-invoice-converter.1.1.39.js").write_bytes(BUNDLE[:5])
...
def test_content_swapped_for_something_of_equal_length_is_refused(vendor: Path) -> None:
    ...
    (vendor / "src/ksef_mcp/vendor/ksef-fe-invoice-converter.1.1.39.js").write_bytes(swapped)
...
def test_a_missing_bundle_is_refused(vendor: Path) -> None:
    (vendor / "src/ksef_mcp/vendor/ksef-fe-invoice-converter.1.1.39.js").unlink()
...
def test_a_half_finished_version_swap_is_refused_rather_than_guessed(vendor: Path) -> None:
    (vendor / "src/ksef_mcp/vendor/ksef-fe-invoice-converter.1.1.40.js").write_bytes(BUNDLE)
...
def test_a_size_written_with_digit_groups_is_read_as_one_number(vendor: Path) -> None:
    padded = b"y" * 1234
    (vendor / "src/ksef_mcp/vendor/ksef-fe-invoice-converter.1.1.39.js").write_bytes(padded)
    (vendor / "src/ksef_mcp/vendor/LICENCJA-MF.md").write_text(
